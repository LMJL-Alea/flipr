...
p-value function. This article shows how it can be easily computed using the
...
We can compute a point estimate of the mean difference and store it inside the plausibility function class:
```{r}
pfa$set_point_estimates(mean(a2) - mean(a1))
```

It is possible to access the list of parameters under investigation in an object of class `PlausibilityFunction` through the `$param_list` field.
```{r}
pfa$param_list
```

By default, bounds for each parameter are unknown. We can however compute their bounds by defining a maximum confidence level through the `$set_max_conf_level()` method of the `PlausibilityFunction` class. When a plausibility function is instantiated, the default value for the `$max_conf_level` field is $0.99$. To set parameter bounds automatically, use the `$set_parameter_bounds()` method:
```{r}
pfa$set_parameter_bounds(
  point_estimate = pfa$point_estimates, 
  conf_level = pfa$max_conf_level
)
```

We can now inspect the list of parameters under investigation to see the updated bounds:
```{r}
pfa$param_list
```
...
pfa$set_grid(
  point_estimate = pfa$point_estimates, 
  range_list = get_ranges(pfa$param_list), 
  npoints = 50L
)
```
...
pfb$set_nperms(B)
pfb$set_point_estimates(mean(b2) - mean(b1))
pfb$set_parameter_bounds(
  point_estimate = pfb$point_estimates, 
  conf_level = pfb$max_conf_level
)
pfb$set_grid(
  point_estimate = pfb$point_estimates, 
  range_list = get_ranges(pfb$param_list), 
  npoints = 50L
)
...
pfc$set_nperms(B)
pfc$set_point_estimates(mean(c2) - mean(c1))
pfc$set_parameter_bounds(
  point_estimate = pfc$point_estimates, 
  conf_level = pfc$max_conf_level
)
pfc$set_grid(
  point_estimate = pfc$point_estimates, 
  range_list = get_ranges(pfc$param_list), 
  npoints = 50L
)
...
$p$-value function (which is $1$). One can use the `$set_point_estimates()` method to do that:
```{r}
pfa$set_point_estimates(overwrite = TRUE)
```

The computed point estimate is then stored in the `$point_estimates` field and can be retrieved as:
```{r}
pfa$point_estimates
```
