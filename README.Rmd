...
We hereby use the very simple t-test for comparing the means of two univariate samples to show how easy it is to carry out a permutation test with **ipa**.
...
We can implement the squared $t$-statistic as a function that plays well with **ipa** as follows:
```{r tstat-ipa}
...
Now we can simply use the function `ipa::two_sample_test()` to get the result of the test:
```{r ttest-ipa}
